...
include "sub_category_processes.php";
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from sub_category where idsub_category=$id");
    $data=mysqli_fetch_array($record);
    $name=$data['name'];
    $category=$data['category_idcategory'];
}
?>
<html>
    <head>
  ...
  <div class="container-fluid">
        <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <?php endif ?>
                <h1>SUB CATEGORY</h1>
                <form class="form-inline" action="sub_category_processes.php" method="POST">
                ...
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from sub_category order by idsub_category desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {?>
                    <tbody>
                    <tr>
                        <td><?php echo $row['idsub_category']; ?></td>
                        <td><?php echo $row["name"]; ?></td>
                        <?php 
                        $cid=$row["category_idcategory"];
                        $qc="select name from category where idcategory='$cid'";
                        $resqc=mysqli_query($conn,$qc);
                        $rowqc=mysqli_fetch_array($resqc);
                        ?>
                        <td><?php echo $rowqc[0]; ?></td>
                        <td><a href="sub_category_index.php?edit=<?php echo $row["idsub_category"];?>" class="edit_btn">edit </a></td>
                        <td><a href="sub_category_processes.php?delete=<?php echo $row["idsub_category"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
