...
include "purchase_details_processes.php";
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from purchase_details where purchase_idpurchase=$id");
...
<div class="container-fluid">
  <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <form class="form-inline" action="purchase_details_processes.php" method="POST">
                ...
                <table>
                    ...
                    $result=mysqli_query($conn,"select * from purchase_details where purchase_idpurchase=$_GET[pid] order by purchase_idpurchase desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {?>
                    <tbody>
                    <tr>
                        <td><?php echo $row['purchase_idpurchase']; ?></td>
                        <?php 
                        $pid=$row['raw_material_idraw_material'];
                        $q="select name from raw_material where idraw_material='$pid'";
                        $resq=mysqli_query($conn,$q);
                        $rowq=mysqli_fetch_array($resq);
                        ?>
                        <td><?php echo $rowq[0]; ?></td>
                        <td><?php echo $row["qty"]."m²"; ?></td>
                        <td><?php echo $row["amount"]; ?></td>
                        <td><a href="purchase_details_index.php?edit=<?php echo $row["purchase_idpurchase"];?>&pid=<?php echo $row["purchase_idpurchase"];?>" class="edit_btn">edit </a></td>
                        <td><a href="purchase_details_processes.php?delete=<?php echo $row["purchase_idpurchase"];?>&rid=<?php echo $row["raw_material_idraw_material"]?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
