...
// Use Ajax
fpurchase_return_reportrpt.lists["x_name"] = <?php echo $purchase_return_report_rpt->name->Lookup->toClientList() ?>;
fpurchase_return_reportrpt.lists["x_name"].popupValues = <?php echo json_encode($purchase_return_report_rpt->name->SelectionList) ?>;
fpurchase_return_reportrpt.lists["x_name"].popupOptions = <?php echo JsonEncode($purchase_return_report_rpt->name->popupOptions()) ?>;
fpurchase_return_reportrpt.lists["x_name"].options = <?php echo JsonEncode($purchase_return_report_rpt->name->lookupOptions()) ?>;
...
<div class="input-group">
	<select class="custom-select ew-custom-select" data-table="purchase_return_report" data-field="x_name" data-value-separator="<?php echo $Page->name->displayValueSeparatorAttribute() ?>" id="x_name" name="x_name"<?php echo $Page->name->editAttributes() ?>>
...
<input type="text" data-table="purchase_return_report" data-field="x_date" id="x_date" name="x_date" maxlength="10" placeholder="<?php echo HtmlEncode($Page->date->getPlaceHolder()) ?>" value="<?php echo HtmlEncode($Page->date->AdvancedSearch->SearchValue) ?>"<?php echo $Page->date->editAttributes() ?>>
...
<?php PrependClass($Page->date->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="purchase_return_report" data-field="x_date" id="y_date" name="y_date" maxlength="10" placeholder="<?php echo HtmlEncode($Page->date->getPlaceHolder()) ?>" value="<?php echo HtmlEncode($Page->date->AdvancedSearch->SearchValue2) ?>"<?php echo $Page->date->editAttributes() ?>>
<?php if (!$purchase_return_report_base->date->ReadOnly && !$purchase_return_report_base->date->Disabled && !isset($purchase_return_report_base->date->EditAttrs["readonly"]) && !isset($purchase_return_report_base->date->EditAttrs["disabled"])) { ?>
<script>
...
<hr class="bg-dark">
<center><h2><u>Purchase Return Report</u></h2></center>
<!-----My code end--->
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="name"><div class="purchase_return_report_name"><span class="ew-table-header-caption"><?php echo $Page->name->caption() ?></span></div></td>
<?php } else { ?>
	<td data-field="name">
<?php if ($Page->sortUrl($Page->name) == "") { ?>
		<div class="ew-table-header-btn purchase_return_report_name">
			<span class="ew-table-header-caption"><?php echo $Page->name->caption() ?></span>
	<?php if (!$DashboardReport) { ?>
			<a class="ew-table-header-popup" title="<?php echo $ReportLanguage->phrase("Filter"); ?>" onclick="ew.showPopup.call(this, event, { id: 'x_name', form: 'fpurchase_return_reportrpt', name: 'purchase_return_report_name', range: false, from: '<?php echo $Page->name->RangeFrom; ?>', to: '<?php echo $Page->name->RangeTo; ?>' });" id="x_name<?php echo $Page->Counts[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer purchase_return_report_name" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->name) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->name->caption() ?></span>
...
<?php if ($Page->sortUrl($Page->date) == "") { ?>
	...
	<?php if (!$DashboardReport) { ?>
			<a class="ew-table-header-popup" title="<?php echo $ReportLanguage->phrase("Filter"); ?>" onclick="ew.showPopup.call(this, event, { id: 'x_date', form: 'fpurchase_return_reportrpt', name: 'purchase_return_report_date', range: true, from: '<?php echo $Page->date->RangeFrom; ?>', to: '<?php echo $Page->date->RangeTo; ?>' });" id="x_date<?php echo $Page->Counts[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer purchase_return_report_date" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->date) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->date->caption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="name1"><div class="purchase_return_report_name1"><span class="ew-table-header-caption"><?php echo $Page->name1->caption() ?></span></div></td>
<?php } else { ?>
	<td data-field="name1">
<?php if ($Page->sortUrl($Page->name1) == "") { ?>
		<div class="ew-table-header-btn purchase_return_report_name1">
			<span class="ew-table-header-caption"><?php echo $Page->name1->caption() ?></span>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer purchase_return_report_name1" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->name1) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->name1->caption() ?></span>
...
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer purchase_return_report_qty" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->qty) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->qty->caption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="reason"><div class="purchase_return_report_reason"><span class="ew-table-header-caption"><?php echo $Page->reason->caption() ?></span></div></td>
<?php } else { ?>
	<td data-field="reason">
<?php if ($Page->sortUrl($Page->reason) == "") { ?>
		<div class="ew-table-header-btn purchase_return_report_reason">
			<span class="ew-table-header-caption"><?php echo $Page->reason->caption() ?></span>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer purchase_return_report_reason" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->reason) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->reason->caption() ?></span>
