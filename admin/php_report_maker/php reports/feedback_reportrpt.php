...
// Use Ajax
ffeedback_reportrpt.lists["x_name"] = <?php echo $feedback_report_rpt->name->Lookup->toClientList() ?>;
ffeedback_reportrpt.lists["x_name"].popupValues = <?php echo json_encode($feedback_report_rpt->name->SelectionList) ?>;
ffeedback_reportrpt.lists["x_name"].popupOptions = <?php echo JsonEncode($feedback_report_rpt->name->popupOptions()) ?>;
ffeedback_reportrpt.lists["x_name"].options = <?php echo JsonEncode($feedback_report_rpt->name->lookupOptions()) ?>;
ffeedback_reportrpt.lists["x_date"] = <?php echo $feedback_report_rpt->date->Lookup->toClientList() ?>;
ffeedback_reportrpt.lists["x_date"].popupValues = <?php echo json_encode($feedback_report_rpt->date->SelectionList) ?>;
ffeedback_reportrpt.lists["x_date"].popupOptions = <?php echo JsonEncode($feedback_report_rpt->date->popupOptions()) ?>;
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown && !$DashboardReport) { ?>
<script>
...
<div class="input-group">
	<select class="custom-select ew-custom-select" data-table="feedback_report" data-field="x_name" data-value-separator="<?php echo $Page->name->displayValueSeparatorAttribute() ?>" id="x_name" name="x_name"<?php echo $Page->name->editAttributes() ?>>
...
<input type="text" data-table="feedback_report" data-field="x_date" id="x_date" name="x_date" maxlength="10" placeholder="<?php echo HtmlEncode($Page->date->getPlaceHolder()) ?>" value="<?php echo HtmlEncode($Page->date->AdvancedSearch->SearchValue) ?>"<?php echo $Page->date->editAttributes() ?>>
<?php if (!$feedback_report_base->date->ReadOnly && !$feedback_report_base->date->Disabled && !isset($feedback_report_base->date->EditAttrs["readonly"]) && !isset($feedback_report_base->date->EditAttrs["disabled"])) { ?>
<script>
ew.createDateTimePicker("ffeedback_reportrpt", "x_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
...
<?php PrependClass($Page->date->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="feedback_report" data-field="x_date" id="y_date" name="y_date" maxlength="10" placeholder="<?php echo HtmlEncode($Page->date->getPlaceHolder()) ?>" value="<?php echo HtmlEncode($Page->date->AdvancedSearch->SearchValue2) ?>"<?php echo $Page->date->editAttributes() ?>>
<?php if (!$feedback_report_base->date->ReadOnly && !$feedback_report_base->date->Disabled && !isset($feedback_report_base->date->EditAttrs["readonly"]) && !isset($feedback_report_base->date->EditAttrs["disabled"])) { ?>
<script>
ew.createDateTimePicker("ffeedback_reportrpt", "y_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
</div>
...
<hr class="bg-dark">
<center><h2><u>feedback Report</u></h2></center>
<!-----My code end--->
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="name"><div class="feedback_report_name"><span class="ew-table-header-caption"><?php echo $Page->name->caption() ?></span></div></td>
<?php } else { ?>
	<td data-field="name">
<?php if ($Page->sortUrl($Page->name) == "") { ?>
		<div class="ew-table-header-btn feedback_report_name">
			<span class="ew-table-header-caption"><?php echo $Page->name->caption() ?></span>
	<?php if (!$DashboardReport) { ?>
			<a class="ew-table-header-popup" title="<?php echo $ReportLanguage->phrase("Filter"); ?>" onclick="ew.showPopup.call(this, event, { id: 'x_name', form: 'ffeedback_reportrpt', name: 'feedback_report_name', range: false, from: '<?php echo $Page->name->RangeFrom; ?>', to: '<?php echo $Page->name->RangeTo; ?>' });" id="x_name<?php echo $Page->Counts[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer feedback_report_name" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->name) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->name->caption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="name1"><div class="feedback_report_name1"><span class="ew-table-header-caption"><?php echo $Page->name1->caption() ?></span></div></td>
<?php } else { ?>
	<td data-field="name1">
<?php if ($Page->sortUrl($Page->name1) == "") { ?>
		<div class="ew-table-header-btn feedback_report_name1">
			<span class="ew-table-header-caption"><?php echo $Page->name1->caption() ?></span>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer feedback_report_name1" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->name1) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->name1->caption() ?></span>
...
<?php if ($Page->sortUrl($Page->date) == "") { ?>
	...
	<?php if (!$DashboardReport) { ?>
			<a class="ew-table-header-popup" title="<?php echo $ReportLanguage->phrase("Filter"); ?>" onclick="ew.showPopup.call(this, event, { id: 'x_date', form: 'ffeedback_reportrpt', name: 'feedback_report_date', range: true, from: '<?php echo $Page->date->RangeFrom; ?>', to: '<?php echo $Page->date->RangeTo; ?>' });" id="x_date<?php echo $Page->Counts[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer feedback_report_date" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->date) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->date->caption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="description"><div class="feedback_report_description"><span class="ew-table-header-caption"><?php echo $Page->description->caption() ?></span></div></td>
<?php } else { ?>
	<td data-field="description">
<?php if ($Page->sortUrl($Page->description) == "") { ?>
		<div class="ew-table-header-btn feedback_report_description">
			<span class="ew-table-header-caption"><?php echo $Page->description->caption() ?></span>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer feedback_report_description" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->description) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->description->caption() ?></span>
