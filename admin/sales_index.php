...
include "sales_processes.php";
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from sales where idsales=$id");
...
<link href="css_panel/admin-2.min.css" rel="stylesheet">
    ...
    function deletefunc()
     ...
     {
        return alert("are you sure you want to send delivery notification?");
     }
...
<!-- 
  ...
  <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <?php endif ?>
                <h1>SALES</h1>
                <form class="form-inline" action="sales_processes.php" method="POST">
                    <input type="hidden" name="id" value="<?php echo $id; ?>"><br>
                    <input type="hidden" name="date" placeholder="date" value="<?php echo date("y/m/d"); ?>"><br>
                ...
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from sales order by idsales desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {
                        $uid=$row['user_iduser'];
                        $sql22="select username from user where iduser= '$uid'";
                        $res1=mysqli_query($conn,$sql22);
                    ...
                    <tr>
                        ...
                        <!-- <td><a href="sales_index.php?edit=<?php //echo $row["idsales"];?>" class="edit_btn">edit </a></td> -->
                        <td><a href="sales_processes.php?delete=<?php echo $row["idsales"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                        <td><a href="sales_details_index.php?sid=<?php echo $row["idsales"];?>" class="price_btn">set&nbsp;products</a></td>
                        <td><a href="delievery_notification.php?sid=<?php echo $row["idsales"];?>" class="price_btn" onclick="notification()">Delievery&nbsp;Notification</a></td>
                        <td><a href="practice_invoice.php?id=<?php echo $row["idsales"];?>&uid=<?php echo $row["user_iduser"];?>" class="edit_btn">generate&nbsp;invoice </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
