...
include "offer_processes.php";
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from offer where idoffer=$id");
...
<div class="container-fluid">
  <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <?php endif ?>
                <h1>Offer</h1>
                <form class="myForm form-inline" name="myForm" id="myForm" action="offer_processes.php" method="POST">
                    <input type="hidden" name="id" value="<?php echo $id; ?>"><br>
                    <label>Start Date(DD-MM-YYYY):</label><br>
                    <input type="date" name="sdate" id="inputDate" name="inputDate" onchange="getDate()" value="<?php echo $sdate; ?>" required><br>
                    <label>End Date(DD-MM-YYYY)</label><br>
                    <input type="date" name="edate" id="inputDate" name="inputDate" onchange="getDate()" value="<?php echo $edate; ?>" required><br>
                    <input type="text" name="discount" placeholder="Discount(%)" value="<?php echo $discount; ?>" required><br>
                ...
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from offer order by idoffer desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {?>
                    <tbody>
                    <tr>
                        <td><?php echo $row['idoffer']; ?></td>
                        <td><?php echo $row["start_date"]; ?></td>
                        <td><?php echo $row["end_date"]; ?></td>
                        <td><?php echo $row["discount"]."%"; ?></td>
                        
                        <td><a href="offer_index.php?edit=<?php echo $row["idoffer"];?>" class="edit_btn">edit </a></td>
                        <td><a href="offer_processes.php?delete=<?php echo $row["idoffer"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
