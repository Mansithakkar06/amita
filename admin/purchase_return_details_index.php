...
//include "raw_material_processes.php";
include "purchase_return_details_processes.php";
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from purchase_return_details where purchase_return_idpurchase_return=$id");
    $data=mysqli_fetch_array($record);
    $qty=$data['qty'];
    $reason=$data['reason'];
    $raw_material=$data['raw_material_idraw_material'];
}
?>
<html>
    <head>
  ...
  <li class="nav-item">
                ...
                <div id="collapseUtilities" class="collapse" aria-labelledby="headingUtilities"
                    ...
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item" href="php_report_maker/php reports/">Go to Reports</a>
                        <!-- <a class="collapse-item" href="utilities-border.html">Purchase</a>
                        <a class="collapse-item" href="utilities-animation.html">Raw-Material</a>
                        <a class="collapse-item" href="utilities-other.html">Production</a>
  ...
  <div class="container-fluid">
        ...
        <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <form class="form-inline" action="purchase_return_details_processes.php" method="POST">
                    <input type="hidden" name="prid" value="<?php echo $_GET['prid']; ?>">
                    <input type="hidden" name="pid" value="<?php echo $_GET['pid']; ?>">
                    <input type="hidden" name="id" value="<?php echo $id; ?>"><br>
                    <input type="text" name="qty" placeholder="qty(m²)" pattern="[0-9]+" value="<?php echo $qty; ?>" required><br>
                    <input type="text" name="reason" placeholder="reason" value="<?php echo $reason; ?>" required><br>
                    <select name="dropdown" required  class="mb-2">
                           <option value disabled selected> --select raw material--</option>
                           <?php
                            $pid=$_GET['pid'];
                              $sql="SELECT * FROM raw_material where idraw_material in(select raw_material_idraw_material from purchase_details where purchase_idpurchase=$pid)";                              ;
                              $result=mysqli_query($conn,$sql);
                              while($row=mysqli_fetch_array($result))
                              {
                                 echo "<option value='".$row['idraw_material']."'>".$row['name']."</option>";
                              }
                           ?>
                    </select>&nbsp;&nbsp;

                    <?php if($edit_state==false): ?>
                        <button class="btn" type="submit" name="save" onclick="return addfunc()">save</button>
                    <?php else: ?>
                        <button class="btn" type="submit" name="update" onclick="return updatefunc()">update</button>
                        <?php endif ?>
                </form>
                ...
                <table>
                    ...
                    <?php
					$prid=$_GET['prid'];
                    $result=mysqli_query($conn,"select * from purchase_return_details where purchase_return_idpurchase_return='$prid' order by purchase_return_idpurchase_return desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {?>
                    <tbody>
                    <tr>
                        <td><?php echo $row['purchase_return_idpurchase_return']; ?></td>
                        <?php 
                        $rid=$row["raw_material_idraw_material"];
                        $qr="select name from raw_material where idraw_material='$rid'";
                        $resqr=mysqli_query($conn,$qr);
                        $rowqr=mysqli_fetch_array($resqr);
                        ?>
                        <td><?php echo $rowqr[0]; ?></td>
                        <td><?php echo $row["qty"]; ?></td>
                        <td><?php echo $row["reason"]; ?></td>
                        <!-- <td><a href="purchase_return_details_index.php?edit=<?php// echo $row["purchase_return_idpurchase_return"];?>&prid=<?//php echo $row["purchase_return_idpurchase_return"];?>" class="edit_btn">edit </a></td> -->
                        <td><a href="purchase_return_details_processes.php?delete=<?php echo $row["purchase_return_idpurchase_return"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
