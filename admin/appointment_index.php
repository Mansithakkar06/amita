...
<link href="css_panel/admin-2.min.css" rel="stylesheet">
  ...
  function confirmfunc()
     {
        return confirm("are you sure you want to confirm the appointment?");
     }
     function donefunc()
     {
        return confirm("are you sure you want to complete the process?");
     }
  </script>
</head>    
<body id="page-top">
  ...
  <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from appointment where status='0' order by idappointment desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {
                        $ida=$row['idappointment'];
                        $res=mysqli_query($conn,"select * from appointment where idappointment= '$ida'");
                        $row1 = mysqli_fetch_array($res);
                        $uid= $row1['user_iduser'];
                        $qusername = mysqli_query($conn, "select username from user where iduser='$uid'");
                        $uname = mysqli_fetch_array($qusername);
                        ?>
                    <tbody>
                    <tr>
                        <td><?php echo $row['idappointment']; ?></td>
                        <td><?php 
                        echo $uname[0];
                        ?></td>
                        <td><?php 
                        ...
                        <td><a href="customized_product_index.php?aid=<?php echo $row["idappointment"];?>" class="edit_btn">Add&nbsp;Products </a></td>
                        <td><a href="appointment_confirm.php?uid=<?php echo $uid;?>" class="edit_btn" onclick="return confirmfunc()">confirm&nbsp;appointment </a></td>
                        <td><a href="appointment_processes.php?delete=<?php echo $row["idappointment"];?>" class="edit_btn" onclick="return donefunc()">done </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
                ...
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from appointment where status='1' order by idappointment desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {
                        $ida=$row['idappointment'];
