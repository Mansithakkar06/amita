...
include "area_processes.php";
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from area where idarea=$id");
    $data=mysqli_fetch_array($record);
    $name=$data['name'];
    $pincode=$data['pincode'];
    $city=$data['city_idcity'];
}
?>
<html>
    <head>
...
<div class="text-center align-items-center mb-4">
    <h1 class="h3 mb-0 text-gray-800 ml-3"></h1>
    <!-- <a href="#" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
        ...
        <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <?php endif ?>
                <h1>AREA</h1>
                <form class="form-inline" action="area_processes.php" method="POST">
                    <input type="hidden" name="id" value="<?php echo $id; ?>"><br>
                    <input type="text" name="name" placeholder="name" value="<?php echo $name; ?>" required><br>
                    <input type="text" name="pincode" placeholder="pincode" pattern="[0-9]{6}" value="<?php echo $pincode; ?>" required><br>
                ...
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from area order by idarea desc");
                    $i=1;
                    ...
                    <tr>
                        ...
                        <?php 
                        $cid=$row["city_idcity"];
                        $qc="select name from city where idcity='$cid'";
                        $resqc=mysqli_query($conn,$qc);
                        $rowqc=mysqli_fetch_array($resqc);
                        ?>
                        <td><?php echo $rowqc[0]; ?></td>
                        
                        <td><a href="area_index.php?edit=<?php echo $row["idarea"];?>" class="edit_btn">edit </a></td>
                        <td><a href="area_processes.php?delete=<?php echo $row["idarea"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
