...
include "raw_material_details_processes.php";
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from raw_material_details where raw_material_idraw_material=$id");
    $data=mysqli_fetch_array($record);
    $price=$data['price'];
    $supplier=$data['supplier_idsupplier'];
}
?><html>
<head>
...
<div class="container-fluid">
        ...
        <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <form class="form-inline" action="raw_material_details_processes.php" method="POST">
                    <input type="hidden" name="rid" value="<?php echo $_GET['setprice']; ?>">
                ...
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from raw_material_details order by raw_material_idraw_material desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {?>
                    <tbody>
                    <tr>
                        <td><?php echo $row['raw_material_idraw_material']; ?></td>
                        <?php 
                        ...
                        <?php 
                        $sid=$row['supplier_idsupplier'];
                        $qs="select name from supplier where idsupplier='$sid'";
                        $resqs=mysqli_query($conn,$qs);
                        $rowqs=mysqli_fetch_array($resqs);
                        ?>
                        <td><?php echo $rowqs[0]; ?></td>
                        <td><?php echo $row["price"]; ?></td>
                        <td><a href="raw_material_details_index.php?edit=<?php echo $row["raw_material_idraw_material"];?>" class="edit_btn">edit </a></td>
                        <td><a href="raw_material_details_processes.php?delete=<?php echo $row["raw_material_idraw_material"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
