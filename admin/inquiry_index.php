...
if(isset($_GET['edit']))
{
    ...
    $pid = $_GET['pid'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from inquiry_details where inquiry_idinquiry=$id and product_idproduct=$pid");
...
<div class="container-fluid">
  <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <?php endif ?>
                <h1>Inquiry</h1>
                <form class="form-inline" action="inquiry_processes.php" method="POST">
                    <input type="hidden" name="id" value="<?php echo $id; ?>"><br>
                    <input type="hidden" name="pid" value="<?php echo $pid; ?>"><br>
                    <input type="text" disabled name="description" placeholder="Inquiry will be here..." value="<?php echo $description; ?>" required><br>
                    <input type="text" name="answer" placeholder="Answer here..." value="<?php echo $answer; ?>"><br>
               ...
               <div class="tablefixhead">
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from inquiry_details order by inquiry_idinquiry desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {
                        $inquiry=$row['inquiry_idinquiry'];
                        $sql2 = "select username from user where iduser in(select user_iduser from inquiry where idinquiry=$inquiry)";
                        $r = mysqli_query($conn, $sql2);
                        $row1 = mysqli_fetch_array($r);
                        $uname = $row1[0];
                        $id = $row['inquiry_idinquiry'];
                        $pid=$row['product_idproduct'];
                        $answer =$row['answer'];
                    ?>
                    <tbody>
                        <tr>
                            <td><?php echo $row['inquiry_idinquiry']; ?></td>
							<?php
							$pq="select name from product where idproduct='$pid'";
							$resp=mysqli_query($conn,$pq);
							$rowp=mysqli_fetch_array($resp)[0];?>
							<td><?php echo $rowp; ?></td>
                            <td><?php echo $uname; ?></td>
                            <td><?php echo $row["description"]; ?></td>
                            <td><input type="text" disabled name="answer" placeholder="Answer..." value="<?php echo $answer; ?>"><br></td>
                            <td><a href="inquiry_index.php?pid=<?php echo $pid;?>&edit=<?php echo $row["inquiry_idinquiry"];?>" class="edit_btn">edit </a></td>
                            <td><a href="inquiry_processes.php?pid=<?php echo $pid;?>&delete=<?php echo $row["inquiry_idinquiry"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                        </tr>
                    </tbody>
                    <?php
                    $i++;
