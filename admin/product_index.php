...
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from product where idproduct=$id");
    $data=mysqli_fetch_array($record);
    $name=$data['name'];
    $price=$data['price'];
    $description=$data['description'];
    $qoh=$data['qoh'];
    $brand=$data['brand_idbrand'];
    $sub_category=$data['sub_category_idsub_category'];
    $offer=$data['offer_idoffer'];
}
?><html>
<head>
...
<div class="container-fluid">
  <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <?php endif ?>
                <h1>PRODUCT</h1>
                <form class="form-inline" action="product_processes.php" method="POST">
                    <input type="hidden" name="id" value="<?php echo $id; ?>"><br>
                    <input type="text" name="name" placeholder="name" value="<?php echo $name; ?>" required><br>
                    <input type="text" name="price" placeholder="price" pattern="[0-9]+" value="<?php echo $price; ?>" required><br>
                    <input type="text" name="description" placeholder="description" value="<?php echo $description; ?>" required><br>
                    <input type="text" name="qoh" placeholder="qoh(m²)" pattern="[0-9]+" value="<?php echo $qoh; ?>" required><br>
                    <select name="dropdown_brand" required class="mb-2">
                    ...
                    <select name="dropdown_sc" required class="mb-2">
                           ...
                           <?php
                              ...
                              while($row=mysqli_fetch_array($result))
                              {
                                 echo "<option value='".$row['idsub_category']."'>".$row['name']."</option>";
                              }
                           ?>
                    </select>&nbsp;&nbsp;
                    <select name="dropdown_offer" required class="mb-2">
                           ...
                           <?php
                              ...
                              while($row=mysqli_fetch_array($result))
                              {
                                 echo "<option value='".$row['idoffer']."'>".$row['idoffer']."</option>";
                              }
                           ?>
                    </select>&nbsp;&nbsp;
                ...
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from product order by idproduct desc");
                    $i=1;
                    ...
                    <tr>
                        ...
                        <?php 
                        $bid=$row['brand_idbrand'];
                        $q="select name from brand where idbrand='$bid'";
                        $resq=mysqli_query($conn,$q);
                        $rowq=mysqli_fetch_array($resq);
                        ?>
                        <td><?php echo $rowq[0]; ?></td>
                        <?php 
                        $sid=$row['sub_category_idsub_category'];
                        $qs="select name from sub_category where idsub_category='$sid'";
                        $resqs=mysqli_query($conn,$qs);
                        $rowqs=mysqli_fetch_array($resqs);
                        ?>
                        <td><?php echo $rowqs[0]; ?></td>
                        <?php 
                        $oid=$row["offer_idoffer"];
                        $qo="select discount from offer where idoffer='$oid'";
                        $resqo=mysqli_query($conn,$qo);
                        $rowqo=mysqli_fetch_array($resqo);
                        ?>
                        <td><?php echo $rowqo[0]."%"; ?></td>
                        <td><a href="product_index.php?edit=<?php echo $row["idproduct"];?>" class="edit_btn">edit </a></td>
                        <td><a href="product_processes.php?delete=<?php echo $row["idproduct"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
