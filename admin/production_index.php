...
include "production_processes.php";
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from production where idproduction=$id");
...
<div class="container-fluid">        <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <?php endif ?>
                <h1>PRODUCTION</h1>
                <form class="form-inline" action="production_processes.php" method="POST">
                ...
                <table>
                    ...
                    <?php
                    $result=mysqli_query($conn,"select * from production order by idproduction desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {?>
                    <tbody>
                    <tr>
                        <td><?php echo $row['idproduction']; ?></td>
                        <td><?php
                        $s="select name from product where idproduct=$row[product_idproduct]";
                        $r=mysqli_query($conn,$s);
                        $pname=mysqli_fetch_array($r)[0];
                        echo $pname;
                        ?></td>
                        <td><?php echo $row["qty"]; ?></td>
                        <td><?php echo $row["date"]; ?></td>
                        
                        <td><a href="production_index.php?edit=<?php echo $row["idproduction"];?>" class="edit_btn">edit </a></td>
                        <td><a href="production_processes.php?delete=<?php echo $row["idproduction"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                        <td><a href="production_details_index.php?setqty=<?php echo $row["idproduction"];?>" class="price_btn">set&nbsp;raw&nbsp;material</a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
