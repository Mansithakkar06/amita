...
include "production_details_processes.php";
if(isset($_GET['edit']))
{
    $id=$_GET['edit'];
    $edit_state=true;
    $record=mysqli_query($conn,"select * from production_details where production_idproduction=$id");
    $data=mysqli_fetch_array($record);
    $raw_material_qty_used=$data['raw_material_qty_used'];
...
<div class="container-fluid">
        ...
        <center>
            <?php if(isset($_SESSION['message'])): ?>
                ...
                <form class="form-inline" action="production_details_processes.php" method="POST">
                    <input type="hidden" name="pid" value="<?php echo $_GET['setqty']; ?>">
                    <input type="hidden" name="id" value="<?php echo $id; ?>"><br>
                    <input type="text" name="raw_material_qty_used" pattern="[0-9]+" placeholder="Raw material qty used(m²)" value="<?php echo $raw_material_qty_used; ?>" required><br>
                ...
                <table>
                    ...
                    <tr>
                        <th>Sr no</th>
                        <th>Raw&nbsp;material</th>
                        <th>Raw&nbsp;material&nbsp;qty&nbsp;used(m²)</th>
                        <th colspan="3">Action</th>
                    </tr>
                    </thead>
                    <?php
                    $result=mysqli_query($conn,"select * from production_details order by production_idproduction desc");
                    $i=1;
                    while($row=mysqli_fetch_assoc($result))
                    {?>
                    <tbody>
                    <tr>
                        <td><?php echo $row['production_idproduction']; ?></td>
                        <?php 
                        $rid=$row['raw_material_idraw_material'];
                        $q="select name from raw_material where idraw_material='$rid'";
                        $resq=mysqli_query($conn,$q);
                        $rowq=mysqli_fetch_array($resq);
                        ?>
                        <td><?php echo $rowq[0]; ?></td>
                        <td><?php echo $row["raw_material_qty_used"]; ?></td>
                        <td><a href="production_details_index.php?edit=<?php echo $row["production_idproduction"];?>" class="edit_btn">edit </a></td>
                        <td><a href="production_details_processes.php?delete=<?php echo $row["production_idproduction"];?>" class="del_btn" onclick="return deletefunc()">delete </a></td>
                    </tr>
                    </tbody>
                    <?php
                    $i++;
