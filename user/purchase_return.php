...
<head>
   ...
   <title>Purchase Return Page</title>
   <!-- bootstrap core css -->
   ...
   <script>
      function myfunc() {
         return confirm("Product Returned Successfully!");
      }
   </script>
            ...
            <table>
               ...
               while ($sales = mysqli_fetch_assoc($res)) {
                  ...
                  while ($sd = mysqli_fetch_assoc($res2)) {
                     ?>
                     <tr>
                     <form action="purchase_return_processes.php" method="post">
                        <td>
                        ...
                        <td> 
                           <input type="number" name="qty" value=1 min=1 max=<?php echo $sd['qty'];?>>
                        ...
                        <td>
                           ...
                           <td><input type="text" name="reason" placeholder="Type your reason...." value=""
                                 style="width:auto;" required></td>
                           <td><button class="now" onclick="myfunc()">Return&nbsp;Product&nbsp;&nbsp;&nbsp;&nbsp;</button></td>
                           <input type="hidden" name="pid" value="<?php echo $sd['product_idproduct']; ?>">
                           <input type="hidden" name="sid" value="<?php echo $sales['idsales']; ?>">
      ...
      <div class="container" style="margin-top:-80px;margin-bottom:-70px;">
         <div class="small-container cart-page">
            <table>
               ...
               $res = mysqli_query($conn, $sql);
               while ($sales = mysqli_fetch_assoc($res)) {
                  $s = "select * from sales_return where sales_idsales=$sales[idsales]";
                  $r = mysqli_query($conn, $s);
                  while ($sr = mysqli_fetch_assoc($r)) {
                     $sql2 = "select * from sales_return_details where sales_return_idsales_return=$sr[idsales_return]";
                     $res2 = mysqli_query($conn, $sql2);
                     while ($sd = mysqli_fetch_assoc($res2)) {
                        ?>
                        <tr>
                           <td>
                              <?php echo $sd['sales_return_idsales_return']; ?>
                           </td>
                           <td>
                              <?php echo $sales['date']; ?>
                           </td>
                           ...
                           <form action="purchase_return_processes.php" method="post">
                              <td><input type="text" disabled name="reason" placeholder="Type your reason...."
                                    value="<?php echo $sd['reason']; ?>" style="width:auto;"></td>
                              <input type="hidden" name="pid" value="<?php echo $sd['product_idproduct']; ?>">
                              <input type="hidden" name="sid" value="<?php echo $sales['idsales']; ?>">
                              <input type="hidden" name="qty" value="<?php echo $sd['qty']; ?>">
                           </form>
